...
const Tasks: React.FC = () => {
  ...
  if (loading) return <div className="p-10 text-center animate-pulse">Retrieving Jira Tickets...</div>;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold">Jira Task Integration</h2>
          <p className="text-sm text-slate-500">{role === 'admin' ? 'Global team backlog overview' : 'Your personal development tasks'}</p>
        </div>
        <button 
                ...
                {tasks.filter(t => t.status === col.status).map(task => (
                  ...
                  >
                    ...
                    <div className="mt-3 flex items-center gap-2 pt-3 border-t border-slate-50 dark:border-slate-700">
                      ...
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-tighter">ID: {task.id}</span>
                    </div>
                  </motion.div>
                ))}
              </AnimatePresence>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-950/50 backdrop-blur-sm">
          ...
          >
            <h3 className="text-xl font-bold mb-6">Create Backlog Task</h3>
